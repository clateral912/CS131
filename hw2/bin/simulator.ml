...
let rec interp_cnd { fo; fs; fz } : cnd -> bool =
  fun x ->
  match x with
  | Eq -> fz
  | Neq -> not fz
...
let fetch_8byte (m : mach) (base_addr : quad) : sbyte list =
  ...
  let rec fetch_nbyte_from_back_core (m : mach) (n : int) (base_addr : int) : sbyte list =
    ...
    then failwith (Printf.sprintf "Memory access out of bounds")
    else (
      match n with
      | 0 -> []
      | _ ->
        fetch_byte (base_addr + n - 1) :: fetch_nbyte_from_back_core m (n - 1) base_addr)
  in
...
let writequad (m : mach) (addr : quad) (w : quad) : unit =
  ...
  let rec write_8byte_from_front (list : sbyte list) (base_addr : int) (offset : int)
    : unit
    =
    if base_addr >= mem_size || base_addr <= 0
    then failwith (Printf.sprintf "Memory access out of bounds")
...
let resolve_value (m : mach) (operand : operand) : int64 =
  ...
  | Ind1 (Lit offset) ->
    readquad m @@ (m.regs.(rind Rip) +. offset) (* TODO: 严重错误！没有考虑string*)
...
let ins_writeback (m : mach) : ins -> int64 -> unit =
  ...
  in
  let update (operand_list : operand list) (res : int64) =
    write_dest res (get_dest operand_list)
  in
  let check_cnd (cnd : cnd) : bool = interp_cnd m.flags cnd in
  fun (instr : ins) (res : int64) : unit ->
    let opcode, operand_list = instr in
    let dest_addr = get_dest operand_list in
    match opcode with
    | Cmpq -> () (* cmpq不涉及任何写入 *)
    | J cnd -> if check_cnd cnd then write_dest res (Reg Rip) else ()
    | Set cnd ->
      (match operand_list with
       | [ dest ] ->
         let original_val =
         ...
         if check_cnd cnd
         then write_dest (cleared_val +. 1L) dest
         else write_dest cleared_val dest
...
let set_flags (m : mach) (op : opcode) (ws : quad list) (w : Int64_overflow.t) : unit =
  ...
  let update_sf_zf (res : Int64_overflow.t) : unit =
  ...
  let update_shift (res : Int64_overflow.t) (op : opcode) : unit =
    ...
    in
    if amt_val <> 0L then update_sf_zf w;
    match op with
    | Sarq -> if amt_val = 1L then resetOF () else ()
    | Shlq ->
      if amt_val = 1L
      then (
...
let step (m : mach) : unit =
  ...
  List.iter
    (fun ((uop, _) as u) ->
      ...
      if uop = Popq || uop = Pushq then () else set_flags m op ws res)
    uops
;;
...
let is_size (is : ins list) : quad =
  let rec get_size (is : ins list) (cnt : quad) : quad =
    match is with
    | [] -> cnt
    | h :: tl -> get_size tl (Int64.add cnt 8L)
    (* 每个指令的长度为8byte *)
  in
  get_size is 0L
;;
...
let rec build_symbol_table (p : prog) (text_pos : quad) (data_pos : quad)
  ...
  | elem :: tl ->
    (match elem.asm with
     | Text text ->
       (elem.lbl, text_pos) :: build_symbol_table tl (text_pos +. is_size text) data_pos
     | Data data ->
       (elem.lbl, data_pos) :: build_symbol_table tl text_pos (data_pos +. ds_size data))
;;

let if_duplicate_label (symbol_table : (lbl * quad) list) =
  let compare_label ((label1, _) : lbl * quad) ((label2, _) : lbl * quad) =
    Stdlib.compare label1 label2
  in
  let sorted_table = List.sort compare_label symbol_table in
  let rec find_duplicate (table : (lbl * quad) list) : unit =
    (* TOLEARN: 以下这个模式匹配值得学习 *)
    match table with
    | [] | [ _ ] -> ()
    | h1 :: h2 :: tl ->
      let (l1, _), (l2, _) = h1, h2 in
      if l1 = l2
      then raise (Redefined_sym l1)
      else (* 这个“黏合”列表的操作非常厉害 *)
        find_duplicate (h2 :: tl)
  in
